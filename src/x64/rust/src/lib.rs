...
pub fn ascon128_encrypt(m: Vec<u8>, a: Vec<u8>, n: [u8; 16], k: [u8; 16]) -> (Vec<u8>, u64) {
    let _m = align(&m);
    let _a = align(&a);
...
pub fn ascon128_decrypt(c: Vec<u8>, a: Vec<u8>, n: [u8; 16], k: [u8; 16]) -> (Vec<u8>, u64) {
    if c.len() < 16 {
        panic!();
    }
...
pub fn ascon128a_encrypt(m: Vec<u8>, a: Vec<u8>, n: [u8; 16], k: [u8; 16]) -> (Vec<u8>, u64) {
    let _m = align(&m);
    let _a = align(&a);
    let mut _c = vec![0; _m.len() + 16];
...
pub fn ascon128a_decrypt(c: Vec<u8>, a: Vec<u8>, n: [u8; 16], k: [u8; 16]) -> (Vec<u8>, u64) {
    if c.len() < 16 {
        panic!();
    }
    let _c_tag = [&c[(c.len()-16)..], &c[..(c.len()-16)]].concat();
